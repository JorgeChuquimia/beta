...
<body>
    ...
    <h4 class="mx-auto">Mi Perfil</h4>
    <form action="reporte_per_prof.php" method="post" style="display: inline;">
...
<!-- Botón para agregar nuevo título -->
<div class="text-right mb-3">
    <?php
    // Asegúrate de que $id_prof está definido y no es nulo
    if (isset($id_prof) && $id_prof !== null) {
        // Codifica el id_prof para la URL
        $id_prof_encoded = urlencode($id_prof);

        ?>
        <a href="agregar_titulo.php?id_prof=<?php echo $id_prof_encoded; ?>" class="btn btn-success">Agregar Nuevo Título</a>
        <?php
    } else {
        ?>
        <p class="text-danger">No se pudo obtener el id_prof. Asegúrate de estar correctamente registrado como profesor.</p>
        <?php
    }
    ?>
</div>
    ...
    if (count($titulos) > 0) {
        ...
        foreach ($titulos as $titulo) { ?>
            <tr>
                ...
                <td>
                    <a href='include/eli_tit.php?id_tit=<?php echo $titulo['id_tit']; ?>' 
